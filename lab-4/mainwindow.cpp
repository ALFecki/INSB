...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow),
      dbConnection(PSQLDBHelper("QPSQL")), login(LoginWindow(&dbConnection)) {
  ui->setupUi(this);
  ...
  connect(this->ui->refactorButton, &QPushButton::clicked, this,
          &MainWindow::refactor);
}
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (column == 2 && value.contains(QRegularExpression("[a-zA-Z]"))) {
      QMessageBox::warning(this, "Error",
                           "In this column only numbers are available!");
      return;
    }
    QString columnName;
    if (column == 1) {
    ...
    if (dbConnection.updateRow(
            this->ui->usersTable->model()->data(index).toInt(), columnName,
            value)) {
