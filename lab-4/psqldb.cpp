...
int PSQLDBHelper::getUserRole(QString login, QString password) {
  QSqlQuery query;
  query.prepare("SELECT role_id FROM users WHERE login = :login AND password = "
                ":password;");
...
QList<User> PSQLDBHelper::getAllUsers() {
  ...
  if (res.has_value()) {
    ...
    do {
      users.append(User(
          query.value("id").toInt(), query.value("login").toString(),
...
bool PSQLDBHelper::updateRow(int id, QString field, QString value) {
  QSqlQuery query;
  query.prepare(
      QString("UPDATE users SET %1 = :value WHERE id = :id;").arg(field));
