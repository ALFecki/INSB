...
PSQLDBHelper::PSQLDBHelper(const char* driver) { db = new QSqlDatabase(QSqlDatabase::addDatabase(driver)); }

bool PSQLDBHelper::connectToDatabase(const QString& server, const QString& databaseName, const QString& userName,
...
QList<User> PSQLDBHelper::getAllUsers() {
    ...
    if (res.has_value()) {
        ...
        do {
            users.append(User(query.value("id").toInt(), query.value("login").toString(),
                              query.value("password").toString(), query.value("role_id").toInt()));
        } while (values->next());
    }
    qDebug() << users.size();
    return users;
