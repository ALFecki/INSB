...
public:
    ...
    bitset<28> leftShift(const bitset<28>& bits, int rotateCount);

    void generateKeys();

    bitset<64> change(bitset<64> temp);

    bitset<64> DES_encrypt(bitset<64>& plain);

    bitset<64> DES_decrypt(bitset<64>& cipher);

    void getText(string str) {
        text = str;
    }

    void setKey(string key_t) {
        this->key = key_t;
    }

    bitset<64> charToBit(const char s[8]);

    string encryptAnyString(string plaintext);

    string decryptAnyString(string cipher);
...
private:
    ...
    int IP[64] = {58, 50, 42, 34, 26, 18, 10, 2,  60, 52, 44, 36, 28, 20, 12, 4,  62, 54, 46, 38, 30, 22,
                  14, 6,  64, 56, 48, 40, 32, 24, 16, 8,  57, 49, 41, 33, 25, 17, 9,  1,  59, 51, 43, 35,
                  27, 19, 11, 3,  61, 53, 45, 37, 29, 21, 13, 5,  63, 55, 47, 39, 31, 23, 15, 7};

    int IP_1[64] = {40, 8,  48, 16, 56, 24, 64, 32, 39, 7,  47, 15, 55, 23, 63, 31, 38, 6,  46, 14, 54, 22,
    ...
    int PC_1[56] = {57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51, 43,
